...
class TD2:
    ...
    def _convert_line(self, line: str) -> np.ndarray:
        hoge = line.replace("S", str(self.start)).replace("G", str(self.goal)).replace(" ", str(self.road)).replace("#", str(self.wall))
        return hoge
    ...
    def start_pos(self) -> tuple[int, int]:
        start_arr = np.where(self.field == TD2.start)
        return (start_arr[0][0], start_arr[1][0])

    def is_wall(self, pos: tuple[int, int]) -> bool:
        return self.field[pos] == TD2.wall

    def is_goal(self, pos: tuple[int, int]) -> bool:
        return self.field[pos] == TD2.goal
...
if __name__ == "__main__":
    ...
    seed(100)
    field_path = Path("/home/gkinoshita/workspace/lectures/reinforcement/field.txt")
    td = TD2(field_path, epsilon=epsilon)
